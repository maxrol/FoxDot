...
# I think this kind of imports is a little bit dangerous because we cannot see
# what is imported and may cause some troubles...
if sys.version_info[0] > 2:
    from .OSC3 import *
else:
    from .OSC import *
...
class SCLangServerManager(ServerManager):
    ...
    # Maybe here we can avoid to declare variables already declared in the ServerManager class.
...
class RequestHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        assert "init" in data  # assert is not used for unit tests?
...
class TempoClient:
    ...
    # I know that doing music using computers, there is latency, so it is a good idea to calculate it.
